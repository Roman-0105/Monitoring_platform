...
-- На одном PLOT_NAME не должно быть двух одновременно активных схем —
-- обеспечивает то же "одна схема на участок", что уже реализовано в
-- клиенте (upload заменяет существующую строку, а не плодит новые).
CREATE UNIQUE INDEX UX_SCHEMES_PLOT_ACTIVE ON dbo.GEOLOCATION_SCHEMES(PLOT_NAME) WHERE DELETED = 0;
```
...
> боевом API этого ограничения нет: изображение, как и фото обращений,
...
Это то, что должен делать конкретно этот блок API — три операции,
завязанные на файл на диске плюс строку в БД:

**Создание / замена изображения — `POST /schemes/:plotId`**
...
2. Сохранить присланный файл на диск/файловый сервер (тот же принцип,
   ...
   `scheme_<plotId>_<timestamp>.jpg`).
3. Найти существующую строку `GEOLOCATION_SCHEMES` для этого `plotId`
   (`WHERE PLOT_NAME = :plotId AND DELETED = 0`):
   - если есть — **удалить со диска старый файл** (по старому значению
     `IMAGE`), обновить `IMAGE`, `UPLOADED_AT`, `UPLOADED_BY`,
     `RECORD_VERSION += 1`. Границы (`X_MIN..Y_MAX`) не трогать —
     заменяется только картинка, калибровка обычно остаётся актуальной,
     если новый скан того же масштаба;
   - если нет — вставить новую строку с `X_MIN..Y_MAX = NULL`.
4. **Не** удалять старый файл, пока новый не записан успешно (сначала
   пишем новый, потом чистим старый) — если что-то упадёт посередине,
   не должно быть ситуации "старого файла уже нет, а нового ещё нет".

**Изменение границ — `PUT /schemes/:plotId/bounds`**
Просто `UPDATE X_MIN, X_MAX, Y_MIN, Y_MAX, RECORD_VERSION += 1
WHERE PLOT_NAME = :plotId AND DELETED = 0`. Если строки нет — `404`
("сначала загрузите изображение", как и в клиенте).

**Удаление — `DELETE /schemes/:plotId`**
1. `UPDATE GEOLOCATION_SCHEMES SET DELETED = 1 WHERE PLOT_NAME = :plotId AND DELETED = 0`
   (soft-delete строки, как и везде в этой БД).
...
3. Вернуть `204`. Клиент после этого покажет на карте состояние
   "схема не загружена" — точки того участка временно не отображаются,
   пока не загрузят новую схему (это ожидаемое поведение, не баг).
...
| `GET /schemes/:plotId` | текущая схема участка (или `null`) | — | `{id, plotName, image, xMin, xMax, yMin, yMax, uploadedAt, uploadedBy}` |
| `POST /schemes/:plotId` | загрузить/заменить изображение схемы (замена — со старым файлом на диске, см. раздел 2.4) | `{image, uploadedBy}` (base64 или уже загруженный файл) | запись |
| `PUT /schemes/:plotId/bounds` | сохранить границы координат (ручной ввод или расчёт калибровки) | `{xMin, xMax, yMin, yMax}` | запись |
| `DELETE /schemes/:plotId` | удалить схему участка (см. раздел 2.4 — включая файл на диске) | — | `204` |
