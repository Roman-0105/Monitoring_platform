...
-- участка (вкладка "Карта", кнопка 🧩 "Слои" — там только чекбоксы
-- показать/скрыть слой целиком, без создания/редактирования геометрии
-- из этой панели). Координаты — та же локальная система X/Y, что и
...
-- статичные геологические особенности, а не еженедельный срез. Сейчас
-- (тестовая модель) эти таблицы заполняются один раз при сидировании
-- (см. risk-inspection/seed-geology.js — реальные координаты карьера
-- ЮРГ, перенесённые из проекта "Гидрогеологический мониторинг").
...
> **Вкладка «Отчёты» (ui-reports.js)** формирует сводку по обращениям
> (KPI, разбивки по уровню/риску, просроченные «СТОП!», снимки карты по
> неделям) **только по клику** и **только новыми REST-эндпоинтами не
> пользуется** — целиком собрана из уже перечисленного контракта
> (`GET /calllog`, `GET /calllog/:id/actions`, справочники, `GET /schemes/...`,
> `GET /faults/...`, `GET /domains/...`, `GET /colors/...`). Автоматическая
> периодическая рассылка отчётов по расписанию (например, «каждый
> понедельник в 9:00» руководителю) сознательно не реализована — при
> желании её можно добавить позже тем же способом, что и e-mail по риску
> выше (нужен только SMTP + планировщик на сервере, набор данных для
> самого отчёта уже есть).
...
| `GET /faults/:plotId` | список разломов участка — единственный вызов, которым сейчас реально пользуется UI (вкладка «Карта», чекбокс «Разломы») | — | `[{id, plotName, name, points:[[x,y],...], createdAt, createdBy}, ...]` |
| `POST /faults/:plotId` | создать разлом | `{points:[[x,y],...], name?, createdBy}` | запись |
| `DELETE /faults/item/:id` | удалить разлом | — | `204` |
| `GET /domains/:plotId` | список доменов участка — используется вкладкой «Карта» | — | `[{id, plotName, name, color, points:[[x,y],...], createdAt, createdBy}, ...]` |
| `GET /domains` | все домены по всем участкам сразу — используется вкладкой «Настройка цветов» | — | тот же объект + `plotLabel` (название участка) |
| `POST /domains/:plotId` | создать домен | `{points:[[x,y],...], name?, color?, createdBy}` | запись |
| `PUT /domains/item/:id` | перекрасить домен — единственная запись, которой реально пользуется UI (вкладка «Настройка цветов») | `{name?, color?}` | запись |
| `DELETE /domains/item/:id` | удалить домен | — | `204` |

> `POST`/`DELETE` для разломов и доменов текущим UI не вызываются —
> панель «Слои» (🧩) на вкладке «Карта» умеет только показать/скрыть
> уже существующий слой целиком, без создания и удаления отдельных
> фигур (см. README.md). Эндпоинты оставлены в контракте как часть
> модели данных `RiskApi.faults`/`RiskApi.domains` (data.js) — на
> случай, если появится отдельный административный инструмент
> редактирования геологии; реализовывать их прямо сейчас не обязательно,
> если приоритет — только чтение существующих слоёв.
