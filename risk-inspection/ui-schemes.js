...
async function renderSchemeContent(panelEl) {
  ...
  contentEl.innerHTML =
    ...
    '</div>' +
    (scheme
      ? '<img class="ri-scheme-preview" id="ri-sch-preview-img" src="' + escAttr(scheme.imageUrl) + '" alt="Схема участка">' +
...
function openCalibrationModal(plotId, weekKey, scheme, onSaved) {
  var body =
    ...
    '<div class="ri-cal-wrap" id="ri-cal-wrap">' +
      '<img id="ri-cal-img" src="' + escAttr(scheme.imageUrl) + '" draggable="false">' +
